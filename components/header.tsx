          ...
          <nav className="hidden lg:flex items-center gap-1">
            {navItems?.map((item) => (
              ...
              >
                <Link
                  href={item?.href ?? '#'}
                  className="flex items-center gap-1 px-4 py-2 text-sm font-medium text-neutral-700 hover:text-primary-dark transition-colors rounded-lg hover:bg-neutral-100"
                >
                  {item?.label ?? ''}
                      ...
                      {item?.dropdown?.map((sub) =>
                        sub?.external ? (
                          <a
                            ...
                            className="block px-4 py-2 text-sm text-neutral-700 hover:text-primary-dark hover:bg-neutral-50 transition-colors"
                          >
                            {sub?.label ?? ''}
                          </a>
                        ) : (
            ...
            <nav className="max-w-container mx-auto px-6 py-4 space-y-2">
              {navItems?.map((item) => (
                <div key={item?.label ?? 'mobile-nav-item'}>
                  <Link
                    href={item?.href ?? '#'}
                    className="block px-4 py-2 text-sm font-medium text-neutral-700 hover:text-primary-dark"
                  ...
                  {item?.dropdown?.map((sub) =>
                    sub?.external ? (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="block px-8 py-2 text-sm text-neutral-500 hover:text-primary-dark"
